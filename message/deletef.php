...
else
{
	...
	//
	DBConnect();
	$query1 = $db->Execute("SELECT MESSAGEID FROM {$TBLName}offer WHERE MESSAGEID=$messageID");
	$query2 = $db->Execute("SELECT MESSAGEID FROM {$TBLName}invoice WHERE MESSAGEID=$messageID");
	...
	{
		$query4 = "DELETE FROM {$TBLName}message WHERE MESSAGEID=$messageID";
