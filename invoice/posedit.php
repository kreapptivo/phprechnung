...
{
	$query1 = $db->Execute("SELECT P.POSITIONID, P.POS_NAME, T.POSITIONID, T.POS_QUANTITY, T.POS_DESC, T.POS_PRICE, T.TAX, T.TAX_MULTI, T.TAX_DIVIDE, T.TAX_DESC, T.POS_GROUP, T.TMP_INVOICEID FROM {$TBLName}article AS P, {$TBLName}tmp_invoice AS T WHERE T.POSITIONID=P.POSITIONID AND TMP_INVOICEID=$tmpPosID");
}
