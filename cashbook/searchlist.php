...
if (!$query)
	...
	if(isset($Canceled) && $Canceled == "1")
	{
		$query1 = $db->Execute("SELECT TAKINGS, EXPENDITURES FROM {$TBLName}cashbook WHERE CANCELED=1 AND CASHBOOKID LIKE '$CashbookNo_1%' AND TAKINGS LIKE '%$Takings_1%'
	...
	{
		$query1 = $db->Execute("SELECT TAKINGS, EXPENDITURES FROM {$TBLName}cashbook WHERE CASHBOOKID LIKE '$CashbookNo_1%' AND TAKINGS LIKE '%$Takings_1%'
		AND EXPENDITURES LIKE '%$Expenditures_1%' AND DESCRIPTION LIKE '%$Description_1%' AND CASHBOOK_DATE >= '$DateFrom_1' AND CASHBOOK_DATE <= '$DateTill_1'");
	}
	else
	{
		$query1 = $db->Execute("SELECT TAKINGS, EXPENDITURES FROM {$TBLName}cashbook WHERE CANCELED=2 AND CASHBOOKID LIKE '$CashbookNo_1%' AND TAKINGS LIKE '%$Takings_1%'
		AND EXPENDITURES LIKE '%$Expenditures_1%' AND DESCRIPTION LIKE '%$Description_1%' AND CASHBOOK_DATE >= '$DateFrom_1' AND CASHBOOK_DATE <= '$DateTill_1'");
	}
...
//
$query3 = $db->Execute("SELECT CASH_IN_HAND_STARTING_WITH, TAKINGS, EXPENDITURES, CASHBOOK_DATE FROM {$TBLName}cashbook WHERE CANCELED=2 AND CASHBOOK_DATE >= '$Min_Cashbook_Date' AND CASHBOOK_DATE <= '$DateTill_1'");
...
//
$query4 = $db->Execute("SELECT CASH_IN_HAND_STARTING_WITH, TAKINGS, EXPENDITURES, CASHBOOK_DATE FROM {$TBLName}cashbook WHERE CANCELED=2 AND CASHBOOK_DATE >= '$Min_Cashbook_Date' AND CASHBOOK_DATE < '$DateFrom_1'");
