...
//
$query = $db->Execute("SELECT CATEGORYID, DESCRIPTION FROM {$TBLName}category WHERE DESCRIPTION LIKE '%$Description_1%' ORDER BY $Order $Sort LIMIT $intCursor, $EntrysPerPage");
...
if (!$query)
	...
	// Count MaxRows
	//
	$query1 = $db->Execute("SELECT CATEGORYID, DESCRIPTION FROM {$TBLName}category WHERE DESCRIPTION LIKE '%$Description_1%'");
