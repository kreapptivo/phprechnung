...
if (isset ( $PosID ) && is_numeric ( $PosID ) && ! empty ( $PosID )) {
	$PosID = $PosID;
	$smarty->assign ( "PosID", $PosID );
	$query1 = $db->Execute ( "SELECT P.POSITIONID, P.POS_NAME, P.POS_DESC, P.POS_PRICE, P.POS_TAX AS TAX, P.POS_GROUP, T.TAXID, T.TAX_MULTI, T.TAX_DIVIDE, T.TAX_DESC FROM {$TBLName}article AS P, {$TBLName}tax AS T WHERE P.POS_TAX=T.TAXID AND P.POSITIONID=$PosID" );
} else {
	$query1 = $db->Execute ( "SELECT P.POSITIONID, P.POS_NAME, T.POSITIONID, T.POS_QUANTITY, T.POS_DESC, T.POS_PRICE, P.POS_TAX AS TAX, T.TAX_MULTI, T.TAX_DIVIDE, T.TAX_DESC, P.POS_GROUP, T.TMP_INVOICEID FROM {$TBLName}article AS P, {$TBLName}tmp_invoice AS T WHERE T.POSITIONID=P.POSITIONID AND TMP_INVOICEID=$tmpPosID" );
}
