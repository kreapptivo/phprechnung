...
if (isset ( $tmpPos ) && $tmpPos == '1') {
	...
	if (isset ( $MethodOfPayment )) {
		$query = $db->GetRow ( "SELECT METHODOFPAYID, PAYMENT_NOTE, DESCRIPTION, TEXT_ONLY, DAYS FROM {$TBLName}methodofpay WHERE METHODOFPAYID=$MethodOfPayment" );
		$METHOD_OF_PAY = $query ['DESCRIPTION'];
		$DAYS = $query ['DAYS'];
		$PAYMENT_NOTE = $query ['PAYMENT_NOTE'];
	...
	$query = $db->Execute ( "SELECT A.PREFIX, A.TITLE, A.FIRSTNAME, A.LASTNAME, A.ADDRESS, A.COMPANY, A.POSTALCODE, A.PRINT_NAME,A.CITY, A.COUNTRY, A.METHODOFPAY, A.MYID,
		DATE_FORMAT(I.INVOICE_DATE,'%d.%m.%Y') AS INVOICE_DATE, I.INVOICEID, I.PAID, I.SUM_PAID, I.TOTAL_AMOUNT, I.MYID,I.CREATEDBY, I.MESSAGE_DESC, I.NOTE,
		I.TAX1_TOTAL, I.TAX2_TOTAL, I.TAX3_TOTAL, I.TAX4_TOTAL, I.TAX1_DESC, I.TAX2_DESC, I.TAX3_DESC, I.TAX4_DESC, I.SUBTOTAL1, I.SUBTOTAL2, I.SUBTOTAL3, I.SUBTOTAL4,
		P.PAYMENT_NOTE, P.TEXT_ONLY, P.DAYS, P.DESCRIPTION AS METHOD_OF_PAY, DATE_FORMAT(I.METHOD_OF_PAY_DATE,'%d.%m.%Y') AS METHOD_OF_PAY_DATE
		FROM {$TBLName}addressbook AS A,{$TBLName}invoice AS I LEFT JOIN
		{$TBLName}methodofpay AS P using (METHODOFPAYID)
		WHERE A.MYID=I.MYID AND I.INVOICEID=$invoiceID" );
...
if (isset ( $tmpPos ) && $tmpPos == '1') {
	$posquery = $db->Execute ( "SELECT P.POSITIONID, P.POS_NAME, P.POS_UNIT, T.USERNAME, T.POSITIONID, T.POS_DESC, T.POS_QUANTITY, T.POS_PRICE, T.TAX, T.TAX_DIVIDE, T.TAX_MULTI, T.TAX_DESC, T.POS_GROUP, T.TMP_INVOICEID FROM {$TBLName}article AS P, {$TBLName}tmp_invoice AS T WHERE P.POSITIONID=T.POSITIONID AND T.USERNAME='$_SESSION[Username]' ORDER BY T.POS_GROUP ASC, T.TMP_INVOICEID ASC" );
} else {
	$posquery = $db->Execute ( "SELECT P.POSITIONID, P.POS_NAME, P.POS_UNIT, V.POSITIONID, V.POS_DESC, V.INVOICEPOSID,
