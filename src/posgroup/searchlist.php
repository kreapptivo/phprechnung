...
//
$query = $db->Execute("SELECT POSGROUPID, DESCRIPTION FROM {$TBLName}posgroup WHERE DESCRIPTION LIKE '%$Description_1%' ORDER BY $Order $Sort LIMIT $intCursor, $EntrysPerPage");
...
if (!$query)
	...
	// Count MaxRows
	//
	$query1 = $db->Execute("SELECT POSGROUPID, DESCRIPTION FROM {$TBLName}posgroup WHERE DESCRIPTION LIKE '%$Description_1%' ORDER BY $Order $Sort");
