...
if (isset ( $tmpPos ) && $tmpPos == '1') {
	$posquery = $db->GetAll ( "SELECT P.POSITIONID, P.POS_NAME, P.POS_UNIT, T.USERNAME, T.POSITIONID, T.POS_DESC, T.POS_QUANTITY, T.POS_PRICE, T.TAX, T.TAX_DIVIDE, T.TAX_MULTI, T.TAX_DESC, T.POS_GROUP, T.TMP_OFFERID FROM {$TBLName}article AS P, {$TBLName}tmp_offer AS T WHERE P.POSITIONID=T.POSITIONID AND T.USERNAME='$_SESSION[Username]' ORDER BY T.POS_GROUP ASC, T.POS_DESC ASC" );
} else {
	$posquery = $db->GetAll ( "SELECT P.POSITIONID, P.POS_NAME, P.POS_UNIT, V.POSITIONID, V.POS_DESC,
