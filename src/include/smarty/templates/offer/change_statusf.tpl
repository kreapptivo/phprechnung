...
<div align="center">
<form action="change_status.php?{$Session}" method="post">
<input type="hidden" name="offerID" value="{$offerID}" />
<input type="hidden" name="myID" value="{$myID}" />
<input type="hidden" name="infoID" value="{$infoID}" />
<input type="hidden" name="page" value="{$page}" />
<input type="hidden" name="STATUS" value="{$OfferStatus}" />
{if $infoID eq 9}
...
<input type="submit" title="{$BackMsg} - {$offerID} - {$Change_Status}" value="{$BackMsg}" /></form>
</div>
