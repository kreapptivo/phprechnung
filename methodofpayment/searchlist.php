...
//
$query = $db->Execute("SELECT METHODOFPAYID, DESCRIPTION FROM {$TBLName}methodofpay WHERE DESCRIPTION LIKE '%$Description_1%' ORDER BY $Order $Sort LIMIT $intCursor, $EntrysPerPage");
...
if (!$query)
	...
	// Count MaxRows
	//
	$query1 = $db->Execute("SELECT METHODOFPAYID, DESCRIPTION FROM {$TBLName}methodofpay WHERE DESCRIPTION LIKE '%$Description_1%' ORDER BY $Order $Sort");
