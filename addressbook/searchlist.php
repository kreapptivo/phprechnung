...
//
$query = $db->Execute("SELECT LASTNAME, FIRSTNAME, COMPANY, PHONEWORK, MYID, CREATEDBY FROM {$TBLName}addressbook
...
if (!$query)
	...
	// Count search result
	//
	$query1 = $db->Execute("SELECT LASTNAME, FIRSTNAME, COMPANY, PHONEWORK, MYID, CREATEDBY FROM {$TBLName}addressbook
		WHERE FIRSTNAME LIKE '%$Customer%' OR LASTNAME LIKE '%$Customer%' OR COMPANY LIKE '%$Customer%'");
